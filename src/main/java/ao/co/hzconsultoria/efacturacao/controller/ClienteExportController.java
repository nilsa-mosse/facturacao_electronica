...
public class ClienteExportController {
        ...
        try {
            ...
            if (busca != null && !busca.trim().isEmpty()) {
                ...
                clientes = clientes.stream()
                        .filter(c -> (c.getNome() != null && c.getNome().toLowerCase().contains(b))
                                || (c.getNif() != null && c.getNif().toLowerCase().contains(b))
                                || (c.getEmail() != null && c.getEmail().toLowerCase().contains(b)))
            ...
            switch (formato.toLowerCase()) {
                case "json":
                    return ResponseEntity.ok()
                            ...
                            .body(clientes);
                case "excel":
                    return exportToCsv(clientes);
                case "pdf":
                    return exportToPdf(clientes);
                default:
    ...
    private ResponseEntity<byte[]> exportToCsv(List<Cliente> clientes) {
        ...
        for (Cliente c : clientes) {
            csv.append(c.getId()).append(";")
               .append(c.getNome() != null ? c.getNome() : "").append(";")
               .append(c.getNif() != null ? c.getNif() : "").append(";")
               .append(c.getEmail() != null ? c.getEmail() : "").append(";")
               .append(c.getTelefone() != null ? c.getTelefone() : "").append(";")
               .append(c.getEndereco() != null ? c.getEndereco() : "").append("\n");
        }

        byte[] out = csv.toString().getBytes(StandardCharsets.UTF_8);
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Disposition", "attachment; filename=clientes.csv");
        headers.add("Content-Type", "text/csv; charset=utf-8");
        return new ResponseEntity<>(out, headers, HttpStatus.OK);
    }

    private ResponseEntity<byte[]> exportToPdf(List<Cliente> clientes) throws Exception {
        ...
        leftCell.addElement(new Paragraph("Listagem Oficial de Clientes", titleFont));
        ...
        Paragraph p2 = new Paragraph("Total: " + clientes.size() + " cliente(s)", totalFont);
        ...
        String hashOriginal = "Clientes-" + nifEmpresa + "-" + formattedDate;
        ...
        pdfHeaders.add("Content-Disposition", "attachment; filename=clientes.pdf");
